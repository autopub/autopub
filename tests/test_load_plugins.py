...
from autopub.cli.plugins import PluginNotFoundError, find_plugins


def test_finds_builtin_by_short_name():
    plugins = find_plugins(["bump_version"])
...
def test_finds_builtin_plugin_via_extended_path():
    plugins = find_plugins(["autopub.plugins.bump_version"])
...
def test_raises_if_plugin_not_found_valid_module(temporary_working_directory: Path):
    ...
    with pytest.raises(PluginNotFoundError):
        find_plugins(["example2"])


def test_raises_if_plugin_not_found():
    with pytest.raises(PluginNotFoundError):
        find_plugins(["not_found"])


def test_supports_passing_plugin_class():
    plugins = find_plugins(["autopub.plugins.bump_version:BumpVersionPlugin"])
